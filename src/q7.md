```js 
const clusterData = await FileAttachment("data/country_clusters.csv").csv();
const worldData = await fetch("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(d => d.json());

const dataMap = new Map();
clusterData.forEach(d => {
  dataMap.set(d.country, String(d.cluster));
});

// Chart setup
const width = 800, height = 500, margins = 0;
...
For this map, we used a K-Means algorithm to divide the countries into clusters based on their socioeconomic status. Cluster 0, represented by orange, are the developing economies with high literacy rates and are transitioning towards a higher economic output. Cluster 1, represented by blue, are lower income countries that rely on renewable energy. Cluster, represented by red, 2 are high income countries that also have both massive energy consumption and greenhouse gas emissions. Finally, cluster 3, represented by green, are countries that have high inflation and whose economies are stable. 
From the map, it looks like there is a clear divide between Asia and South America and the African countries. In Africa, it looks like the majority of those countries are lower income countries but using incredibly high amonuts of renewable energy. Compared to Asia and South America, whose economies are developing, many African countries have not yet started to sacrifice the environment for economic goals. 
