...
const clusterLabels = {
    ...
    "1": "Low-Income (Renewable)",
    "2": "High-Emissions Outliers",
    "3": "High-Income Developed"
};
...
chart.append("g")
  ...
  .join("path")
    ...
    .attr('fill', d => {
        ...
        return data ? clusterColors[String(data.cluster)] : "#e0e0e0";
    })
    .style("stroke", "#333")
    ...
    .on("mouseover", function(event, d) {
        ...
        if (!data) {
            tooltip.style("opacity", 1)
                    .html(`<b>${d.properties.name}</b><br/>Insufficient Data`);
            return;
        }
        
        const cluster = clusterLabels[String(data.cluster)];
        
        let content = `<b>${d.properties.name}</b><br/>Cluster: ${cluster}`;
        
        if (data.gdp !== undefined) {
            content += `<br/>GDP per capita: $${d3.format(",.0f")(+data.gdp)}`;
            content += `<br/>Renewable Energy: ${d3.format(".1f")(+data.renewable)}%`;
            content += `<br/>GHG Emissions: ${d3.format(".2f")(+data.ghg)} t CO2e/capita`;
            content += `<br/>Inflation: ${d3.format(".1f")(+data.inflation)}%`;
        }
    ...
    .on("mouseout", function() {
        ...
        d3.select(this).style("stroke-width", 0.5).style("stroke", "#333");
    });
...
clusters.forEach((c, i) => {
    ...
    row.append("text")
        ...
        .attr("font-size", "12px")
        .style("fill", "#f0f0f0")
        .text(clusterLabels[c]);
});

// Add "No Data" entry
const noDataRow = legend.append("g")
    .attr("transform", `translate(0, ${clusters.length * 20})`);
